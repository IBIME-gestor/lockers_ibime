...
// Alias aceptados por columna (sin acentos, en minúsculas) para tolerar
// variaciones en el archivo que suba el usuario.
const ALIAS = {
...
export async function parsearExcelAlumnos(archivo) {
  ...
  if (filas.length === 0) {
    return { alumnos: [], errores: ['El archivo no contiene filas de datos.'] }
  }
  ...
  const requeridos = ['matricula', 'nombre', 'grupoEspanol', 'grupoIngles', 'tutorNombre']
  ...
  if (faltantes.length > 0) {
    errores.push(
      `No se encontraron las columnas: ${faltantes.join(', ')}. Verifica los encabezados del Excel.`
    )
